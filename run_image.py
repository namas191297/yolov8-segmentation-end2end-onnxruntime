...
def apply_masks_and_draw(nms_output, individual_masks, image, fps, original_width, original_height, input_width, input_height):
    """
    Applies masks to the input image, draws bounding boxes, and displays the FPS.
    ...
    for i, detection in enumerate(nms_output):
        # Parse the bounding box and rescale to the original image size
...
def main(args):
    """
    Main function to handle image input, ONNX model inference, and applying masks.
    ...
    session_options.log_severity_level = 4  # Only critical errors will be shown
    ...
    try:
        ...
        if nms_output_with_scaled_boxes_and_masks.shape[0] != 0:
            ...
            # Apply masks and draw on the original image
            apply_masks_and_draw(nms_output_with_scaled_boxes_and_masks, final_masks, image, fps, original_width, original_height, model_input_width, model_input_height)
        else:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Object detection and segmentation with ONNX model on an image.")
